...
function scoreSemifinalExactGoals(prediction: any, real: any) {
  let points = 0;

  const matches = ["M101","M102"];
...
function scoreThirdPlaceExactGoals(prediction: any, real: any) {

  const predicted = prediction?.knockout?.["M103"];
  const realMatch = real?.knockout?.["M103"];

  if (!predicted || !realMatch) return 0;
...
function scoreFinalExactGoals(prediction: any, real: any) {
  const predicted = prediction?.knockout?.["M104"];
  const realMatch = real?.knockout?.["M104"];
